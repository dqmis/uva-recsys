...
@hydra.main(version_base=None, config_path="../conf", config_name="xlnet")
def main(cfg):
    data_args = HfArgumentParser(DataArguments).parse_dict(cfg.data_args)[0]
    model_args = HfArgumentParser(ModelArguments).parse_dict(cfg.model_args)[0]
    training_args = HfArgumentParser(TrainingArguments).parse_dict(
        cfg.training_args, allow_extra_keys=True
    )[0]
    ...
    if (
        model_args.input_features_aggregation
    ...
    infer_embedding_sizes = not model_args.input_features_aggregation.startswith(
        "element-wise"
    )
    ...
    model_body = t4r.SequentialBlock(
        input_module,
        # t4r.MLPBlock([64]),
        t4r.TransformerBlock(model_config, masking=input_module.masking),
    )
    model_head = t4r.Head(
        model_body,
        prediction_task,
    ...
    # Extract label
    labels = prediction_data["article_id-list"].apply(lambda x: x[-1]).values
    ...
    if is_dataframe_empty(eval_path):
        print("TEST DATAFRAME IS EMPTY")
        return

    # Get top-10 predictions
    test_loader = get_loader(
        Dataset(prediction_data), training_args, data_args, schema, train=False
    )
...
def get_model_schema(data_args, train_args, schema):
    data_path = glob.glob(
        os.path.join(
            data_args.data_path,
            str(data_args.start_time_window_index),
            "valid.parquet",
        )
    )
...
def incremental_train_eval(
    ...
    for time_index in range(start_time_index, end_time_index):
        ...
        if training_args.do_train:
            ...
            if is_dataframe_empty(train_paths):
                print("DATAFRAME IS EMPTY")
                continue

            trainer.train_dataloader = get_loader(
                train_paths, training_args, data_args, schema, train=True
            )
        ...
        if training_args.do_eval:
            ...
            trainer.eval_dataloader = get_loader(
                train_paths, training_args, data_args, schema, train=False
            )
            ...
            if is_dataframe_empty(eval_paths):
                print("DATAFRAME IS EMPTY")
                continue

            # 4. Evaluate on valid/test data of time_index+1
            trainer.eval_dataloader = get_loader(
                eval_paths, training_args, data_args, schema, train=False
            )
